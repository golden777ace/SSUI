...
### Всплывающие сообщения и позиция окон
- `win.push(title, text, *, secs=3.0, w=200, h=150, x=None, y=None, bg, fg, font=None, size=16.0, rad=8.0, pd=10.0, gp=6.0, click=True, on_close=None)`
- Безрамочное окно поверх всех, без кнопок и без строки в панели задач, по центру монитора вызвавшего окна
- Гаснет через `secs` и по клику; `secs=0` — до клика или `close`; возвращает окно, его можно закрыть или дополнить через `build`
- Цвета, семейство и размер шрифта действуют только на это окно; заголовок на 15 % крупнее текста, оба переносятся по словам
- Требует живого цикла сообщений: вызывать из колбэка, таймера или `post`, но не до `go`
- `subwin(..., center="screen" | "parent" | False, x=None, y=None)` — центр монитора по рабочей области, центр родителя либо позиция от системы
- `center=True` теперь означает `"screen"`; явные `x` и `y` сильнее `center` и задаются оба сразу
