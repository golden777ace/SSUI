...
class TreeRow(TypedDict, total=False):
    ...
    text: str
    leaf: bool
    open: bool  # необязателен: без него состояние переживает пересборку
    values: list[str]
    bg: str
    fg: str
    icon: str
...
class Note:
    ...
    def __call__(
        ...
        *,
        ...
        """Уведомление в углу окна: `tl`, `tr`, `bl`, `br`.

        Отсчёт идёт от углов окна, а не экрана: уведомление живёт
        внутри того окна, у которого вызван `nt`. Чтобы сообщение
        не потерялось за подокном, шлите его из главного окна.
        """
    ...
    def snack(
        ...
        *,
        ...
        С `corner` встаёт в указанный угол и складывается в общую
        стопку с обычными уведомлениями того же угла.
        """
        """Снэкбар внизу окна."""
...
class W:
    ...
    def enable(self, n: Node, on: bool = True) -> None:
        """Доступность узла и всего его поддерева.

        Отключённый узел не ловит мышь, не берёт фокус и рисуется
        приглушённым. Достаточно отключить контейнер — всё, что
        внутри, отключается вместе с ним.

        Значение виджета при этом сохраняется, и `val`, `txt`,
        `chk` продолжают работать: отключение закрывает ввод от
        пользователя, а не от программы.
        """
    def pwd(self, n: Node, on: bool = True) -> None:
        """Показывает содержимое `tx` или `ta` точками.

        Маскируется только отрисовка: `sig`, `txt` и чтение значения
        отдают настоящий текст. Поэтому переключение ascii/hex при
        включённой маске больше не портит значение — маска физически
        не попадает в поле.
        """
    ...
    def lim(
        ...
        *,
        ...
        """Пределы размеров узла в пикселях.

        `None` оставляет прежнее значение, ноль в максимуме снимает
        ограничение. Пределы сильнее всего: они применяются после
        `w`/`h`, доли `grow`, ячейки сетки и авто-высоты.

        Работают во всех раскладках, поэтому кнопка не сожмётся
        ниже `min_w`, а таблица со своим `min_w` не даст соседям
        отобрать место. У областей разделителя пределы задают ход
        рукоятки: `spl` не пустит её за `min_w` любой из половин.

        Узел без явного предела берёт его из содержимого: вдоль
        своей оси — суммой минимумов детей и зазоров, поперёк —
        максимумом, плюс отступы и шапка. `tbl` и `tre` отдают
        наверх сумму минимумов колонок. Поэтому достаточно задать
        `cols(mins=...)`, и разделитель уже не перекроет таблицу:
        предел поднимется через панель сам.

        Явное значение подъём отменяет — берётся именно оно, даже
        если содержимому нужно больше. Ноль везде означает «нет
        предела», так что без единого `lim` и `mins` раскладка
        работает ровно как раньше. Прокрутка при нехватке места
        не появляется: если она нужна, оберните узел в `scr`.

        Безопасно вызывать из колбэков и после показа окна.
        """
    def spl_ratio(self, n: Node, ratio: float) -> None:
        """Задаёт долю первой области разделителя, 0..1.

        Крайние значения означают полное схлопывание области, но
        итоговая доля поджимается её минимумом — в том числе поднятым
        из содержимого. Область с таблицей, у которой заданы `mins`
        колонок, до нуля не сожмётся: прочитанное после раскладки
        значение будет больше заданного.

        Чтобы убрать область целиком, снимите с неё минимум либо
        уберите её из раскладки через `show(pane, False)`. Скрытая
        область отдаёт соседу всё место вместе с полосой рукоятки.
    ...
    def hotkey(self, spec: str, f: Callable[[], None]) -> None:
        ...
        # У `tre` есть `on_open(index, open)` — приходит и по клику
        # на треугольник, и из `tre_open`, но не при пересборке `bind`.
        #
        # Раскрытие переживает пересборку: если в `TreeRow` нет ключа
        # `open`, строка забирает прежнее состояние узла с той же
        # глубиной и подписью. Сначала сверяется строка на том же
        # месте, затем — первая подходящая по всему дереву, так что
        # добавление, удаление и перестановка соседей раскрытие не
        # ломают. Ключ `open` в строке остаётся принудительной
        # установкой и перекрывает сохранённое.
        #
        # Правый клик по `tre` сначала выделяет строку под курсором — так
        # же, как левый, — и лишь затем вызывает `rmb`. Если строка уже
        # входит в выделение, оно сохраняется целиком; если под курсором
        # пусто, выделение снимается. Поэтому в обработчике достаточно
        # читать своё состояние, обновлённое `ch`, а координаты нужны
        # только для позиции меню.
    ...
    def menu(
        self, items: list[str], *, on_select: Optional[Callable[[int], None]] = None
    ) -> None:
        """Контекстное меню окна по правому клику.

        Действует на всё окно: ПКМ в любом месте, где нет своего
        обработчика `rmb`, открывает эти пункты. Для меню, привязанного
        к узлу, оставьте список пустым и вызывайте `popup` из `rmb`.
        """
    def popup(
        self,
        items: list[str],
        ...
        *,
        ...
        Задумано под `rmb(node, f)`: обработчик получает координаты и
        сам решает, какие пункты показать, — так меню оказывается
        привязано к конкретному узлу, а не ко всему окну.

        `on_select(index)` можно задать здесь либо один раз в `menu`;
        последнее заданное значение и работает. Обработчик ставится
        на корень, поэтому вызывайте `popup` с `on_select` или `menu`
        до показа окна хотя бы раз — дальше меняйте только пункты.

        Меню открывается в начале следующего кадра и подтягивается
        внутрь окна, если у края не хватает места. Пункты живут только
        до закрытия и оконное меню из `menu` не затирают.
        """
    ...
    def grp(
        ...
        *,
        ...
        # Содержимое `scr` обрезается по его рамке: то, что уехало за
        # верх или низ области, не рисуется и не ловит мышь. Обрезка
        # наследуется вложенными узлами, поэтому `scr` внутри `tab`
        # больше не заезжает на полосу вкладок.
    def scr(
        self,
        *,
        pr: Optional[Node] = None,
    ...
    def cols(
            ...
            *,
            ...
            on_cols: Optional[Callable[[list[float]], None]] = None,
    ) -> None:
        """Ширины и минимумы колонок таблицы или дерева, в пикселях.

        `widths` — фиксированные ширины; ноль или отсутствие элемента
        означает «поделить остаток поровну». `mins` — минимумы: ниже
        них колонка не сожмётся ни при каком размере узла, и таблица
        начнёт уезжать под собственную прокрутку.

        Минимум действует и на фиксированную колонку: она получит
        `max(width, min)`. Ноль в `mins` возвращает умолчание — 40 px
        для делимых колонок и полную свободу для фиксированных.

        Список короче числа колонок дополняется умолчаниями.
        Безопасно вызывать из колбэков и после показа окна.

        `head` задаёт высоту шапки; ноль возвращает умолчание в
        34 px. Шапка выше умолчания переносит длинный заголовок по
        словам, так что многострочный текст пишется и как перенос,
        и через `\\n`.

        Границы колонок в шапке тянутся мышью: захват ±4 px, ход
        ограничен минимумами обеих соседних колонок, а их общая
        ширина сохраняется — таблица не разъезжается. Первое же
        перетаскивание фиксирует все колонки в их текущих пикселях.
        Двойной клик по границе подгоняет колонку под содержимое.

        `resize=False` запрещает перетаскивание. `on_cols(widths)`
        приходит после отпускания и после автоширины — им удобно
        сохранять раскладку колонок в настройках; обработчик нужно
        задать до показа окна.
        """
    ...
    def tre_opened(self, node: Node) -> list[int]:
        """Индексы раскрытых строк дерева.

        Возвращает только узлы-группы; листья в список не попадают.
        Значение читается сразу, поэтому вызывать нужно из колбэка
        или из тела кадра, а не до показа окна.
        """

    def tre_open(self, node: Node, index: int, on: bool = True) -> None:
        """Раскрывает или сворачивает строку дерева.

        Вызывает `on_open`, если состояние действительно изменилось.
        Безопасно вызывать из колбэков и после показа окна.
        """
        # Клик по пустой области `tre` снимает выделение: одиночное дерево
        # получает `ch(-1)`, дерево с `multi=True` — `ch([])`. То же
        # приходит при правом клике мимо строк.
    def tre(
        self,
        ...
        *,
        ...
        on_open: Optional[Callable[[int, bool], None]] = None,
        pd: float = 0.0,
        gp: float = 0.0,
        w: Optional[float] = None,
        h: Optional[float] = None,
