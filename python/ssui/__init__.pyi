...
class W:
    ...
    def spl_ratio(self, n: Node, ratio: float) -> None:
        """Задаёт долю первой области разделителя, 0.1..0.9.

        Итоговая доля дополнительно поджимается пределами областей,
        поэтому прочитанное после раскладки значение может отличаться
        от заданного. Безопасно вызывать из колбэков и после показа.
        """
