...
class W:
    ...
    def subwin(
        ...
        *,
        thm: str = "drk",
        modal: bool = False,
        center: Union[bool, Literal["screen", "parent"]] = "screen",
        frameless: bool = False,
        topmost: bool = False,
        resizable: bool = True,
        minbox: bool = False,
        ...
        `center="screen"` ставит окно по центру монитора, на котором
        находится родитель, — по рабочей области, без панели задач.
        `"parent"` центрирует по прямоугольнику родителя, как раньше.
        `False` отдаёт позицию системе.

        `center=True` теперь означает `"screen"`: у окна, вылезшего
        за край монитора, кнопки оказывались за пределами экрана,
        и центр монитора почти всегда нужнее центра родителя.

        Явные `x` и `y` сильнее `center`: задаются оба или ни одного,
    ...
    def push(
        ...
        """Сообщение поверх всех окон, независимое от родителя.

        Безрамочное окно без кнопок и без строки в панели задач,
        по центру монитора, на котором стоит вызвавшее окно. `x` и
        `y` ставят его в любую точку экрана; задаются оба сразу.

        Гаснет через `secs` секунд и по клику, если `click`. При
        `secs=0` живёт до клика или до `close`. `on_close` приходит
        при любом закрытии.

        Цвета, семейство и размер шрифта задаются здесь и действуют
        только на это окно: заголовок печатается на 15 % крупнее
        текста, оба переносятся по словам. Тема и стили родителя
        сюда не попадают.

        Фон окна прозрачный, поэтому `rad` скругляет само сообщение,
        а не карточку на чёрной подложке. Ставьте `bg` с альфой
        (`#202632e0`), если нужен просвет.

        Возвращает окно, так что его можно закрыть досрочно через
        `close` или дополнить содержимым через `build`.
